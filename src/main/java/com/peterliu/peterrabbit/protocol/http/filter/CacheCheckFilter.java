    ...
    public boolean doPre(ConfigSource configSource) {
        ...
        if (request.isNeedJudgeCache()) {
            if (request.getUrl().startsWith(Constants.INNER_RESOURCES_PATH)) {
                //内部图片
                if ("inner".equals(request.getMessageDigest())) {
            ...
            if (headers != null && headers.size() > 0) {
                String lastModified = String.valueOf(new Date(file.lastModified()).toString());
                ...
                if ((path = CacheUtils.get(path)) != null &&
                        path.equals(request.getMessageDigest())) {
                    //客户端没有改变,请使用客户端缓存
                    context.setResponse(ResponseFactory.build304Response());
                    return false;
                }
